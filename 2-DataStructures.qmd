...
> the code searches for the specific values at those positions in the vector. once the code finds that value, the code gets rid of that value.
...
> Line 105 -- code is going item by item in the argument. if that item was assigned 'true', it's printed, else it is not printed. since this logic is only appleid to 1:5, 10.2, the last item in the vector, does not show up. Line 110: searches all items in the vector, if this item is greater than 7, it prints.
...
124 code assigns a corresponding letter to the first 4 items in the 'x' vector. in 125, the code is asked to print item d from vector 'y' with its corresponding integer from vector 'x' 
...
> This code is subsetting the first three rows of the data. So, it's giving us all 14 columns, but only 3 variables. 
...
(statepopulations[c(1,6,8,10),c(2,12,13)])
                  
# state populations for the first 4 states listed while eliminating the N/A from the territories to clean up the table if you were searching for change from 1860 -> 1870
```
...
> gayguides part tells the code which dataset, the $city tells the code to look at the subset variable of cities, the [100:110] tells the code to bring out items 100 through 110

(@) Can you find all the titles of locations in South Carolina? (Hint: You'll need to use both `$` and `[]`)
# got stuck here, coming back later
```{r}
