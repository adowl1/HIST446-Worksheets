...
```{r}
paste("Boiler Up", "Hammer Down")
```

(@) Can you explain what happened in 2-3 sentences? 

> ## Code looks to paste the first string here (boiler up). After searching and pasting the phrase, looks for the second string (Hammer down)
...
> 98: Checks to see if 5 equals 10, since false returns false. 99: checks to see if 3 is less than 4, since true returns true. 100: checks to see if 5 does not equal 10, since true returns true. 101: checks to see if 5 does not equal 5, since false returns false. 102: checks to see if 3 is equal to 4 OR 3, since 3 equals 3 returns true. 103: Checks to see if 3 does not equal 4 and if 3 does not equal 5. Since BOTH are true, returns as true.
...
victor.cruz <- "Football Star"
paste(tom.cruise, victor.cruz)
```
...
> # code checks for the item in that list position, then multiplies by 2. For example, looks at integer in position 1 (2), and multiplies by 2 giving 4. Then it checks item 2 (4) multiplies by 2 to get 8, and so on.
...
> code retrieves the third item in the my.numbers list which is 6. then multiplies that by 2. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
my.straight <- c(2,3,4,5,6)
my.straight
```
...
101/5
```
> hmm i honestly don't have a clue. it's cutting off the decimals but i'm unsure about the language it's using in the help screen and how it's different from truncating it.
...
> row z is checking the values of row x and row y and running the equation listed in 331 with those values
...
> I used the help function to figure out how head worked and saw that you could put an integer to get the amount of rows you wanted. In that same help article, it showed tail as a way to get the end of that data.
...
>Gayguides : defining which dataset is being used. $: tells the code to find a specific segment. title: gives the code the specific segment $ is looking for. [500]: displays the 500th item in the "title" segment.
...
```
``` {r}
head(statepopulations,10)
tail(statepopulations,20)
```

(@) What did you learn about the dataset? What can you tell me about it? 

> There are 28 different columns, with 24 of them relating to the census data and population of the territory/state for that census. There's a column called "GISjoin" which may be used for spatial analysis in GIS? The dataset also differentiates between when the state was a territory and gained statehood, and provides or excludes population numbers based on that switch.
